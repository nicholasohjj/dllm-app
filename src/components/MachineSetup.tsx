...
const generateMachines = (): Machine[] => {
  const washers = Array.from({ length: 8 }, (_, i) => {
    ...
    const status: Machine['status'] = isFunctional ? (i % 2 === 0 ? "available" : "in-use") : "complete"; // Explicitly typed status
    return {
      id: machineId,
      shortName: shortName,
  ...
  const dryers = Array.from({ length: 6 }, (_, i) => {
    ...
    const status: Machine['status'] = isFunctional ? (i % 2 === 0 ? "available" : "in-use") : "disabled"; // Explicitly typed status
    return {
      id: machineId,
      shortName: shortName,
      type: "dryer" as const,
